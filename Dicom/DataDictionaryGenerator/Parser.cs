...
namespace ClearCanvas.Dicom.DataDictionaryGenerator
{
    ...
    public class Parser
    {
        ...
        public void ParseFile(String filename)
        {
            ...
            string ns = "{http://docbook.org/ns/docbook}";
            foreach (var chapter in xdoc.Descendants(ns + "chapter"))
            {
                var label = chapter.Attribute("label");
                if (label != null)
                {
                    if (label.Value == "6")
                    {
                        var table = chapter.Descendants(ns + "table").First();
                        var body = table.Descendants(ns + "tbody").First();
                        foreach (var row in body.Descendants(ns + "tr"))
                        {
                            var thisTag = new Tag();
                            var columnArray = row.Descendants(ns + "para").ToArray();
                            if (columnArray.Length == 0)
                                continue;
                            thisTag.tag = columnArray[0].Value.Trim();
                            thisTag.name = columnArray[1].Value.Trim();
                            thisTag.dicomVarName = columnArray[2].Value.Trim();
                            if (columnArray[3] != null)
                                thisTag.vr = columnArray[3].Value.Trim();
                            if (columnArray[4] != null)
                                thisTag.vm = columnArray[4].Value.Trim();
                            if(columnArray.Length == 6)
                                thisTag.retired = columnArray[5].Value.Trim();
                            ...
                            //now we have done all the dicom tags!!1
                        }
                    }
                    ...
                    else if(label.Value == "A")
                    {
                        // all the UIDs
                        var table = chapter.Descendants(ns + "table");
                        var body = table.Descendants(ns + "tbody");
                        foreach (var row in body.Descendants(ns + "tr"))
                        {
                            var columnArray = row.Descendants(ns + "para").ToArray();
                            var thisUid = new SopClass
                            {
                                Uid = columnArray[0].Value.Trim().Trim(new char[] { '\uFEFF', '\u200B' }) ?? string.Empty,
                                Name = columnArray[1].Value.Trim() ?? string.Empty,
                                VarName = columnArray[2].Value.Trim() ?? string.Empty,
                                Type = columnArray[3].Value.Trim() ?? string.Empty
                            };
