...
namespace ClearCanvas.ImageServer.Common.ExternalQuery
{
	...
	/// <summary>
	/// Type provider for <see cref="ImageServerExternalQuery"/> and <see cref="ImageServerExternalQueryResult"/> derives classes.
	/// </summary>
	public static class ImageServerExternalQueryTypeProvider
	{
		private static readonly List<Type> KnownTypes = (from p in Platform.PluginManager.Plugins
		                                                 ...
		                                                 let a = AttributeUtils.GetAttribute<ImageServerExternalQueryTypeAttribute>(t)
		                                                 where (a != null)
		                                                 select t).ToList();
